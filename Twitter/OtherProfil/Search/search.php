		...
		if($count_one > 0){
			while ($info = $donnees_one->fetch()){
				...
				echo '<li class="nav-item">
   						 <a class="nav-link" href="../ClickTweet/clicktweet.php?text='.$hstags.'&id='.$_SESSION['id'].'">'.$info["text"].'</a>
  						</li>';
			}
		}
	}
	...
	public function name(){
		...
		if($count_three > 0){
			while ($info = $donnees_three->fetch()){
				echo '<li class="nav-item">
   						 <a class="nav-link" href="otherProfil.php?id='.$info["id"].'">'.$info["first_name"].'</a>
  						</li>';
			}
		}
	}
